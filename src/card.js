...
export class WeekPlannerCard extends LitElement {
      ...
      this.hass.callApi(
        ...
        response.forEach(event => {
          ...
          if (!fullDay && !this._isSameDay(startDate, endDate)) {
            ...
            this._addEvent(event, startDate, endDate, fullDay, calendar, false, 'start end');
          }
        });
  ...
  _addEvent(event, startDate, endDate, fullDay, calendar, multiDay, position) {
    ...
    if (this._calendarEvents.hasOwnProperty(eventKey)) {
      ...
      this._calendarEvents[eventKey] = {
        ...
        class: this._getEventClass(startDate, endDate, fullDay, multiDay, position)
      }
  ...
  _getEventClass(startDate, endDate, fullDay, multiDay, position) {
    let classes = [];
    let now = DateTime.now();
    if (fullDay) {
  ...
  _handleMultiDayEvent(event, startDate, endDate, calendar) {
    ...
    while (startDate < endDate) {
      ...
      let position = segmentNumber === 0 ? 'start' : '';
      if (eventEndDate.toMillis() === endDate.toMillis()) {
        position = segmentNumber === 0 ? 'start end' : 'end';
      }
